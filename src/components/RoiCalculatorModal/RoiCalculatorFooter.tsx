...
import { Flex, Box, Text, LinkExternal, HelpIcon } from '@fastswap-uikit'
import { ExpandableLabel, Grid, useTooltip } from '@pancakeswap/uikit'
...
const RoiCalculatorFooter: React.FC<RoiCalculatorFooterProps> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of FAST rewards each farm receives, as a proportion of the FAST produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 FAST per block, a 40x farm would receive 40 FAST per block.')}
      </Text>
  ...
  return (
    ...
    //         )}
    //         <Text color="textSubtle" small>
    //           {isFarm ? t('Base APR (FAST yield only)') : t('APR')}
    //         </Text>
    //         <Text small textAlign="right">
    //           {apr.toFixed(2)}%
    //         </Text>
